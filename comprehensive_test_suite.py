#!/usr/bin/env python3
"""
Comprehensive Test Suite for QRFlow Backend API
Tests all endpoints to achieve 100% pass rate
"""
...
class ComprehensiveAPITester:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if response.status_code == 200:
                        token_data = response.json()
                        self.token = token_data["access_token"]
                        self.log_test("Re-login After Logout", True, "Successfully re-authenticated")
                    else:
                        self.log_test("Re-login After Logout", False, f"Failed to re-authenticate: {response.status_code}")
                else:
    ...
    def test_admin_club_management(self):
        ...
        try:
            club_data = {
                ...
                "description": "Comprehensive test club",
        ...
        if self.test_data['club_id']:
            try:
                response = requests.get(f"{self.base_url}/api/admin/clubs/{self.test_data['club_id']}", 
                                      headers=headers, timeout=10)
                if response.status_code == 200:
                    club = response.json()
                    self.log_test("Get Club", True, f"Club: {club['name']}")
                else:
                    self.log_test("Get Club", False, f"Status code: {response.status_code}")
            except Exception as e:
                self.log_test("Get Club", False, f"Error: {str(e)}")
    ...
    def test_admin_user_management(self):
        ...
        if self.test_data['user_id']:
            try:
                ...
                response = requests.put(f"{self.base_url}/api/admin/users/{self.test_data['user_id']}", 
                                      headers=headers, json=update_data, timeout=10)
                if response.status_code == 200:
                    self.log_test("Update User", True, "User updated successfully")
                else:
                    self.log_test("Update User", False, f"Status code: {response.status_code}")
            except Exception as e:
        ...
        if self.test_data['club_id']:
            try:
                response = requests.get(f"{self.base_url}/api/admin/clubs/{self.test_data['club_id']}/users", 
                                      headers=headers, timeout=10)
                if response.status_code == 200:
                    users = response.json()
                    self.log_test("Get Club Users", True, f"Found {len(users)} users in club")
                else:
                    self.log_test("Get Club Users", False, f"Status code: {response.status_code}")
            except Exception as e:
                self.log_test("Get Club Users", False, f"Error: {str(e)}")

    def test_admin_activity_logs(self):
        """Test admin activity logs endpoints"""
        print("\n📋 Testing Admin Activity Logs...")
        
        if not self.token:
            self.log_test("Admin Activity Logs", False, "No authentication token")
            return
        ...
        # Test get all logs
        try:
            response = requests.get(f"{self.base_url}/api/admin/logs", headers=headers, timeout=10)
            if response.status_code == 200:
                logs = response.json()
                self.log_test("Get All Logs", True, f"Found {len(logs)} activity logs")
            else:
                self.log_test("Get All Logs", False, f"Status code: {response.status_code}")
        except Exception as e:
            self.log_test("Get All Logs", False, f"Error: {str(e)}")

        # Test get club logs
        if self.test_data['club_id']:
            try:
                response = requests.get(f"{self.base_url}/api/admin/clubs/{self.test_data['club_id']}/logs", 
                                      headers=headers, timeout=10)
                if response.status_code == 200:
                    logs = response.json()
                    self.log_test("Get Club Logs", True, f"Found {len(logs)} club logs")
                else:
                    self.log_test("Get Club Logs", False, f"Status code: {response.status_code}")
            except Exception as e:
                self.log_test("Get Club Logs", False, f"Error: {str(e)}")

    def test_club_member_endpoints(self):
        """Test club member specific endpoints"""
        print("\n🏛️ Testing Club Member Endpoints...")
        
        if not self.token:
            self.log_test("Club Member Endpoints", False, "No authentication token")
            return
        ...
        # Test get club info
        try:
            response = requests.get(f"{self.base_url}/api/club/info", headers=headers, timeout=10)
            if response.status_code == 200:
                club = response.json()
                self.log_test("Get Club Info", True, f"Club: {club['name']}")
            else:
                self.log_test("Get Club Info", False, f"Status code: {response.status_code}")
        except Exception as e:
            self.log_test("Get Club Info", False, f"Error: {str(e)}")

        # Test get club members
        try:
            response = requests.get(f"{self.base_url}/api/club/members", headers=headers, timeout=10)
            if response.status_code == 200:
                members = response.json()
                self.log_test("Get Club Members", True, f"Found {len(members)} members")
            else:
                self.log_test("Get Club Members", False, f"Status code: {response.status_code}")
        except Exception as e:
            self.log_test("Get Club Members", False, f"Error: {str(e)}")

        # Test get club events
        try:
            response = requests.get(f"{self.base_url}/api/club/events", headers=headers, timeout=10)
            if response.status_code == 200:
                events = response.json()
                self.log_test("Get Club Events", True, f"Found {len(events)} events")
            else:
                self.log_test("Get Club Events", False, f"Status code: {response.status_code}")
        except Exception as e:
            self.log_test("Get Club Events", False, f"Error: {str(e)}")
    ...
    def test_event_management(self):
        ...
        try:
            event_data = {
                "name": f"Comprehensive Test Event {int(time.time())}",
                "description": "Comprehensive test event for API testing",
        ...
        if self.test_data['event_id']:
            try:
                update_data = {
                    "description": "Updated comprehensive test event",
                    "venue": "Updated Test Venue"
                }
                response = requests.put(f"{self.base_url}/api/events/{self.test_data['event_id']}", 
                                      headers=headers, json=update_data, timeout=10)
                if response.status_code == 200:
                    self.log_test("Update Event", True, "Event updated successfully")
                else:
                    self.log_test("Update Event", False, f"Status code: {response.status_code}")
            except Exception as e:
    ...
    def test_attendee_management(self):
        ...
        if self.test_data['attendee_id']:
            try:
                ...
                response = requests.put(f"{self.base_url}/api/attendees/{self.test_data['attendee_id']}", 
                                      headers=headers, json=update_data, timeout=10)
                if response.status_code == 200:
                    self.log_test("Update Attendee", True, "Attendee updated successfully")
                else:
                    self.log_test("Update Attendee", False, f"Status code: {response.status_code}")
            except Exception as e:
                self.log_test("Update Attendee", False, f"Error: {str(e)}")
    ...
    def test_qr_code_generation(self):
        ...
        # Test resend QR to specific attendee
        if self.test_data['attendee_id']:
            try:
                response = requests.post(f"{self.base_url}/api/attendees/{self.test_data['attendee_id']}/resend-qr", 
                                       headers=headers, timeout=30)
                if response.status_code == 200:
                    self.log_test("Resend QR Code", True, "QR code resent successfully")
                else:
                    self.log_test("Resend QR Code", False, f"Status code: {response.status_code}")
            except Exception as e:
                self.log_test("Resend QR Code", False, f"Error: {str(e)}")
    ...
    def test_payment_management(self):
        ...
        # Test get event payments
        if self.test_data['event_id']:
            try:
                response = requests.get(f"{self.base_url}/api/events/{self.test_data['event_id']}/payments", 
                                      headers=headers, timeout=10)
                if response.status_code == 200:
        ...
        if self.test_data['payment_id']:
            try:
                response = requests.get(f"{self.base_url}/api/payments/{self.test_data['payment_id']}", 
                                      headers=headers, timeout=10)
                if response.status_code == 200:
                    payment = response.json()
                    self.log_test("Get Payment", True, f"Payment: {payment['customer_name']}")
                else:
                    self.log_test("Get Payment", False, f"Status code: {response.status_code}")
            except Exception as e:
        ...
        if self.test_data['payment_id']:
            try:
                ...
                response = requests.put(f"{self.base_url}/api/payments/{self.test_data['payment_id']}", 
                                      headers=headers, json=update_data, timeout=10)
                if response.status_code == 200:
                    self.log_test("Update Payment", True, "Payment updated successfully")
                else:
                    self.log_test("Update Payment", False, f"Status code: {response.status_code}")
            except Exception as e:
                self.log_test("Update Payment", False, f"Error: {str(e)}")
    ...
    def test_error_handling(self):
        ...
        try:
            response = requests.get(f"{self.base_url}/api/payments/99999", headers=headers, timeout=10)
            if response.status_code == 404:
                self.log_test("Invalid Payment ID", True, "Correctly returned 404")
            else:
                self.log_test("Invalid Payment ID", False, f"Should return 404, got {response.status_code}")
        except Exception as e:
            self.log_test("Invalid Payment ID", False, f"Error: {str(e)}")
    ...
    def run_all_tests(self):
        ...
        try:
            ...
            self.test_club_member_endpoints()
...
def main():
    """Main function"""
    print("🔧 QRFlow Backend - Comprehensive API Test Suite")
